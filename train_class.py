...
class CustomWithLossCell(nn.Cell):
    ...
    def construct(self, data, label, second_label ,length, target, light, color, composition):
        ...
        return self._loss_fn( label, second_label ,length ,output_class)
...
class CustomWithEvalCell(nn.Cell):
    ...
    def construct(self, data, label, second_label ,length, target, light, color, composition):
        output = self._backbone(data)
        output_class, output_class2, output_light, output_color, output_composition, output_score = self._net(output,light, color, composition)
        return output_class ,label

if __name__ == "__main__":
    
    data_path = "./images/"
    lable_path = "./datalist/train_attributes_mix.csv"
    val_path = "./datalist/valid_attributes_mix.csv"
    ckpt_save_dir = "./class"
    batch_size = 32
    val_batch_size = 64
    epochs = 100
    ...
    ava_train = mydataset.GetDatasetGenerator(data_path, lable_path, divide=divide, transforms=train_transforms,ranges=0.02, gtlabel="score")
    ...
    ava_val = mydataset.GetDatasetGenerator(data_path, val_path, divide=divide, transforms=val_transforms,ranges=0.02, gtlabel="score")
    ...
    load_param_into_net(backdone, param_dict)
    backdone.set_train(True)
    net = Net(10)
    net.set_train(True)
    
    for para in net.trainable_params():
      if para.name.split('.')[0] == 'regression_score1':
        para.requires_grad = False                           
      elif para.name.split('.')[0] == 'regression_score2':
        para.requires_grad = False  
      elif para.name.split('.')[0] == 'regression_color1':
        para.requires_grad = False 
      elif para.name.split('.')[0] == 'regression_color2':
        para.requires_grad = False      
      elif para.name.split('.')[0] == 'regression_light1':
        para.requires_grad = False      
      elif para.name.split('.')[0] == 'regression_light2':
        para.requires_grad = False 
      elif para.name.split('.')[0] == 'regression_composition1':
        para.requires_grad = False 
      elif para.name.split('.')[0] == 'regression_composition2':
        para.requires_grad = False 
      elif para.name.split('.')[0] == 'eca2':
        para.requires_grad = False
      elif para.name.split('.')[0] == 'eca3':
        para.requires_grad = False        
      elif para.name.split('.')[0] == 'eca4':
        para.requires_grad = False
    ...
    param = []
    for para in backdone.trainable_params():
        param.append(para)
    for para in net.trainable_params(): 
        param.append(para)   
    for para in param:
    ...
    polynomial_decay_lr = nn.learning_rate_schedule.PolynomialDecayLR(learning_rate=0.0001,
                                                                  ...
                                                                  decay_steps=4,
                                                                  power=0.5)
    optim = nn.Adam(param, learning_rate=polynomial_decay_lr,beta1=0.98 ,beta2=0.999, weight_decay=0.0001)
    #train_network = nn.SequentialCell([backdone, net])
    ...
    model = Model(loss_net, optimizer=optim,eval_network =eval_net, metrics={"Accuracy": Accuracy()})
    config_ck = CheckpointConfig(save_checkpoint_steps=eval_per_epoch*dataset_size, keep_checkpoint_max=50)
    ckpoint_cb = ModelCheckpoint(prefix="class",directory=ckpt_save_dir, config=config_ck)
