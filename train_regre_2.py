...
class CustomWithLossCell(nn.Cell):
    ...
    def construct(self, data, label, second_label ,length, target, light, color, composition):
        ...
        return self._loss_fn( output_color, target)
...
if __name__ == "__main__":
    ...
    ava_train = mydataset.GetDatasetGenerator(data_path, lable_path, divide=divide, transforms=train_transforms,ranges=0.02, gtlabel="Color")
    ...
    ava_val = mydataset.GetDatasetGenerator(data_path, val_path, divide=divide, transforms=val_transforms,ranges=0.02, gtlabel="Color")
    ...
    param_class = load_checkpoint("./regre1/regre1.ckpt")
    ...
    for para in net.trainable_params():
      ...
      if para.name.split('.')[0] == 'regression_color1':
    ...
    ckpoint_cb = ModelCheckpoint(prefix="regre2",directory=ckpt_save_dir, config=config_ck)
