...
from loss import My_loss,Soft_loss
...
class CustomWithLossCell(nn.Cell):
    def __init__(self, backbone,net, loss_fn1, loss_fn2):
    ...
    def construct(self, data, label, second_label ,length, target, light, color, composition):
        ...
        loss = self._loss_fn1( output_score, target) + self._loss_fn2(output_class, output_class2)*0.1
        return loss
...
if __name__ == "__main__":
    ...
    param_class = load_checkpoint("./regre3/regre3.ckpt")
    ...
    loss_net = CustomWithLossCell(backdone, net, loss1 , loss2)
    cb = LossMonitor()
    ...
    ckpoint_cb = ModelCheckpoint(prefix="regre4",directory=ckpt_save_dir, config=config_ck)
